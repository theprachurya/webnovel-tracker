...
const EditNovel: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNovel = async () => {
      try {
        ...
        if (novel.coverImage) {
          setCoverPreview(`http://localhost:5000${novel.coverImage}`);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/novels/${id}`, formDataToSend, {
        headers: {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
      <h2 className="text-2xl font-serif font-bold mb-6">Edit Novel</h2>
